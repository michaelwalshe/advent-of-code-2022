...
def compute(s: str) -> int:
    ...
    for start, end in itertools.combinations(rooms.keys(), 2):
        ...
        if (
            (start == "AA" or end == "AA")
            or (rooms[start].rate > 0 and rooms[end].rate > 0)
        ):
    ...
    # nodes, only adding paths if they wouldn't go over 30t
    max_flow = 0
    possible_paths = [("AA", 0, 0, set(id for id, v in rooms.items() if v.rate > 0))]
    while possible_paths:
        node, flow, tick, cur_rooms = possible_paths.pop()
        ...
        for room in cur_rooms:
            ...
            if used_ticks < 30:
                possible_paths.append(
                    (
                        room,
                        flow + (30 - used_ticks) * rooms[room].rate,
                        used_ticks,
                        cur_rooms - {room},
                    )
                )
