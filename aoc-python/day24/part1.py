...
def get_neighbours(me, blizzards, walls):
    neighbours = []
    blizz_pnts = set((x, y) for x, y, _ in blizzards)
    for p in (me, *support.adjacent_4(*me)):
        if p not in blizz_pnts and p not in walls and p[1] >= 0:
            neighbours.append(p)
    return neighbours
...
def shortest_path(walls, all_blizzards, start, end, edges):
    ...
    while path_index < len(paths):
        ...
        nt = t + 1
        # next_blizzards = move_blizzards(last_blizzards, edges)
        next_points = get_neighbours(last_point, all_blizzards[nt], walls)
