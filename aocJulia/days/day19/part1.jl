...
function best_geodes(instructions)
        ...
        seen = Set()
        best_geo_at_t = zeros(25)
        while length(todo) > 0
            (t, r, m) = popfirst!(todo)

            m = Materials(
                min(max_ore * (24 - t), m.ore),
                min(obs_clay * (24 - t), m.clay),
                min(geode_obs * (24 - t), m.obs),
                m.geode
            )
...
function compute(s)
    ...
    return sum([id * geo for (id, geo) in blueprints])
end
